...
Expand-Archive prometheus-2.45.0.windows-amd64.zip -DestinationPath C:\prometheus

# Download Alertmanager
wget https://github.com/prometheus/alertmanager/releases/download/v0.25.0/alertmanager-0.25.0.windows-amd64.zip
Expand-Archive alertmanager-0.25.0.windows-amd64.zip -DestinationPath C:\alertmanager
```

### Configure Services
```powershell
# Create Prometheus Windows Service
New-Service -Name "Prometheus" -BinaryPathName "C:\prometheus\prometheus.exe --config.file=E:\Head Ai\monitoring\prometheus\prometheus.yml" -Description "Prometheus Monitoring System" -StartupType Automatic

# Create Alertmanager Windows Service
New-Service -Name "Alertmanager" -BinaryPathName "C:\alertmanager\alertmanager.exe --config.file=E:\Head Ai\monitoring\alertmanager\alertmanager.yml" -Description "Prometheus Alertmanager" -StartupType Automatic
```

### Configure Environment Variables
```powershell
# Set environment variables for Alertmanager
[System.Environment]::SetEnvironmentVariable('SLACK_WEBHOOK_URL', 'your-slack-webhook-url', [System.EnvironmentVariableTarget]::Machine)
[System.Environment]::SetEnvironmentVariable('PAGERDUTY_SERVICE_KEY', 'your-pagerduty-service-key', [System.EnvironmentVariableTarget]::Machine)
```
...
3. Check status of alerts: http://localhost:9090/alerts

### Testing Alerts
1. Create a test alert in Prometheus UI
2. Verify alert appears in Alertmanager
3. Confirm notification delivery to Slack and PagerDuty
