...
def mock_nltk():
    ...
    with patch('nltk.download') as mock_download, \
         patch('nltk.data.path') as mock_path, \
         patch('nltk.word_tokenize') as mock_tokenize, \
...
@pytest.fixture(scope="function")
def nltk_temp_dir(temp_dir):
    """Create a temporary NLTK data directory."""
    nltk_dir = temp_dir / "nltk_data"
    nltk_dir.mkdir(exist_ok=True)
    
    # Create package directories
    for package in ["punkt", "taggers", "corpora"]:
        (nltk_dir / package).mkdir(exist_ok=True)
